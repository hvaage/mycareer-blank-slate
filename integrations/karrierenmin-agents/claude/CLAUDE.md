...
hos Claude er ikke verifisert ende-til-ende. Teksten beskriver semantikken du
skal følge når verktøyene er tilkoblet.

Du har nøyaktig to verktøy: `karrierenmin_status` og `karrierenmin_run`.

## Tilkobling
1. Legg inn MCP-serveren: lagre `mcp.json` som `.mcp.json` i prosjektroten (Claude Code, remote HTTP).
