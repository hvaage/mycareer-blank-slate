...
- RPC `network_link_document_opportunity(document_id, opportunity_id)`: bruker-id er aldri en klientverdi — serverhandlingen henter den fra sesjonen og sender den internt til RPC-en, som validerer at både dokument og mulighet tilhører akkurat denne brukeren før skriving (FK-en er siste sperre, ikke eneste). Tilsvarende avkobling.
- RPC `network_link_posting_contact(opportunity_id, source_contact_ref, existing_contact_id?)`: serveren slår selv opp den lagrede annonsekilden for muligheten og leser navn, rolle, e-post og telefon derfra. `source_contact_ref` er en serverutledet, stabil referanse (annonse-ID + hash av normalisert kontaktdata), ikke en array-indeks som kan flytte seg når kilden endrer rekkefølge. Navn, e-post, telefon, LinkedIn-URL og bruker-id fra klienten aksepteres aldri som kildeverdier. Ved `Koble til eksisterende kontakt` avviser databasen eksplisitt kontakt-ID-er som ikke eies av samme bruker. Ved opprettelse lagres kontaktinfo med `source_class = job_posting`, kilde-ID og observert tidspunkt; manuelle kontaktverdier forblir aktiv verdi og overskrives aldri av senere annonseobservasjon. Navnelikhet kan vises som mulig eksisterende kontakt, men kobling krever brukerens valg.
...
- Alle RPC-er er SECURITY DEFINER uten execute for `anon`/`authenticated`, og kalles kun fra serverhandlinger i `src/lib/network.functions.ts` som henter bruker fra den verifiserte sesjonen. Ingen RPC tar bruker-id fra klientens forespørselskropp.
