DROP INDEX IF EXISTS public.idx_source_postings_title_trgm;

CREATE OR REPLACE FUNCTION public.match_user_opportunities_from_mirror(p_sources text[] DEFAULT ARRAY['careerjet'::text, 'nav'::text], p_max_age_days integer DEFAULT 60, p_limit integer DEFAULT 200)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Forhåndsbygde LIKE-mønstre: unngår korrelert unnest per annonserad,
  -- som var årsaken til statement timeout på ~20k ferske annonser.
  SELECT array_agg('%' || k || '%') INTO v_kw_patterns FROM unnest(v_keywords) k;
  SELECT array_agg('%' || l || '%') INTO v_loc_patterns FROM unnest(coalesce(v_locations, ARRAY[]::text[])) l;
  ...
  WITH ferske AS (
    ...
    WHERE sp.source = ANY (p_sources)
      ...
      AND lower(coalesce(sp.title, '')) LIKE ANY (v_kw_patterns)
  ),
  kandidater AS (
    ...
    WHERE (co.live_until IS NULL OR co.live_until > now())
      AND (
        ...
        OR lower(coalesce(co.display_location, '')) LIKE ANY (v_loc_patterns)
        OR (
          length(coalesce(co.display_location, '')) >= 2
          AND EXISTS (
            SELECT 1 FROM unnest(v_locations) loc
            WHERE loc LIKE '%' || lower(co.display_location) || '%'
          )
        )
      )
      AND NOT EXISTS (
