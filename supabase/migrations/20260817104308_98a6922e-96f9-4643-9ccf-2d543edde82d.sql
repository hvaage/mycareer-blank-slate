...
begin
  ...
  -- Rolleutnevnelser fra v2.1 er autoritet for type og periode.
  with p as (
  ...
  -- Resultater følger rolleutnevnelsen v2.1 knyttet dem til.
  with r as (
  ...
  -- Kandidater brukeren allerede har behandlet rettes aldri automatisk.
  select coalesce(jsonb_agg(jsonb_build_object(
           'local_ref', c.local_ref,
           'status', c.status,
           'promoted_atom_id', c.promoted_atom_id)), '[]'::jsonb)
    into v_blocked
  ...
  where c.import_id = p_import_id
    ...
    and exists (
      ...
      where b.source_id = p_import_id::text
        ...
        and coalesce((pr.proposal_payload->'structured_data'->>'provisional')::boolean,false)
    );
