...
-- Intern hjelper: bygger filterdelene som både søk og telling bruker.
CREATE OR REPLACE FUNCTION public._employer_filter_sql(
...
REVOKE ALL ON FUNCTION public._employer_filter_sql(text,text,text,text,integer,integer,numeric,numeric,text,text,text) FROM PUBLIC, anon, authenticated;

-- Treffantall: eksakt opp til taket, estimat over.
...
GRANT EXECUTE ON FUNCTION public.count_employers(text,text,text,text,integer,integer,numeric,numeric,text,text,text,integer) TO anon, authenticated, service_role;

-- Søket: kandidatvinduet følger nå paginering i stedet for fast tak på 300.
