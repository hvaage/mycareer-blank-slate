-- Fase 4 — LinkedIn-promotering: syntetisk canary.
--
-- Kjøres med:  SELECT * FROM public.linkedin_promotion_phase4_canary();
--
-- Hele testen kjører i en indre subtransaksjon som ALLTID rulles tilbake.
-- Ingen syntetiske rader blir liggende igjen. Testen bruker kun syntetiske
-- bruker-UUID-er, aldri reelle LinkedIn-data.
...
BEGIN
  BEGIN
    ...
    VALUES (imp_a, u_a, 'recommendations'), (imp_a, u_a, 'network'), (imp_a, u_a, 'career');
    INSERT INTO public.linkedin_import_purposes (linkedin_import_id, user_id, purpose)
    VALUES (imp_b, u_b, 'recommendations');
    ...
    VALUES (u_a, imp_a, 'recommendations', 'succeeded', 'sig-p4-1', 3) RETURNING id INTO run_a;
    ...
    VALUES (u_a, run_a, imp_a, 'recommendations', 'recommendations', 'create', 0.9, 'normalized_key', 'rec:1',
    ...
    INSERT INTO public.linkedin_reconciliation_proposals
      (user_id, reconciliation_run_id, linkedin_import_id, purpose, proposal_domain, proposal_kind,
       confidence, match_method, dedupe_key, source_snapshot_json, source_snapshot_hash,
       proposed_payload_json, status, source_classification)
    VALUES (u_a, run_a, imp_a, 'recommendations', 'recommendations', 'create', 0.7, 'normalized_key', 'rec:2',
    ...
    INSERT INTO public.linkedin_reconciliation_proposals
      (user_id, reconciliation_run_id, linkedin_import_id, purpose, proposal_domain, proposal_kind,
       confidence, match_method, dedupe_key, source_snapshot_json, source_snapshot_hash,
       proposed_payload_json, status, source_classification)
    VALUES (u_b, run_a, imp_b, 'recommendations', 'recommendations', 'create', 0.9, 'normalized_key', 'rec:b',
    ...
    -- ---------- 1. ingen promotering uten autentisering ----------
    ...
    -- ---------- 2. beslutningslaget eier keep_existing / manual_edit_required ----------
    ...
    -- ---------- 3. bare godkjente forslag kan promoteres ----------
    ...
    -- ---------- 6. idempotens: samme promotering to ganger ----------
    ...
    -- ---------- 8. atomisk rollback ved skrivefeil ----------
    -- syntetisk bruker finnes ikke i auth.users, så career_atoms-innsettingen må feile
    ...
    -- ---------- 9. hendelsesloggen er append-only ----------
    ...
    -- ---------- 11. produktdata utenfor promoteringsmålene er urørt ----------
    SELECT jsonb_build_object(
      'profiles', (SELECT count(*) FROM public.profiles),
      'user_career_profiles', (SELECT count(*) FROM public.user_career_profiles),
      'career_atoms', (SELECT count(*) FROM public.career_atoms),
