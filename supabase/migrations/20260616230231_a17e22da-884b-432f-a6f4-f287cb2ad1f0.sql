-- M5.4.1: search_employers — kommune ANY(array) → IN (literals), treffsett-bevarende.
BEGIN;

CREATE TEMP TABLE _se_snap_before (label text PRIMARY KEY, n bigint, h text) ON COMMIT DROP;
...
DECLARE
  ...
  cases text[][] := ARRAY[
    ...
    ARRAY['oslo',             'Oslo'],
    ARRAY['bergen',           'Bergen'],
    ARRAY['trondheim',        'Trondheim'],
    ARRAY['tromso',           'Tromsø'],
    ARRAY['prefix_sand',      'sand'],
    ARRAY['no_match_zzzzzzz', 'zzzzzzz']
  ];
  i int;
BEGIN
  FOR i IN 1..array_length(cases,1) LOOP
    EXECUTE
      'SELECT count(*), md5(coalesce(string_agg(organisasjonsnummer, $1 ORDER BY organisasjonsnummer), $2))
       FROM public.search_employers(p_kommune_query => $3::text, p_limit => 100)'
      INTO v_n, v_h USING ',', '', cases[i][2];
    INSERT INTO _se_snap_before(label, n, h) VALUES (cases[i][1], v_n, v_h);
  END LOOP;
END $do$;
...
BEGIN
  ...
  -- M5.4.1: kommunenummer-sett → IN-liste (treffsett-identisk med ANY(array)).
...
CREATE TEMP TABLE _se_snap_after (LIKE _se_snap_before INCLUDING ALL) ON COMMIT DROP;
  ...
  FOR i IN 1..array_length(cases,1) LOOP
    EXECUTE
      'SELECT count(*), md5(coalesce(string_agg(organisasjonsnummer, $1 ORDER BY organisasjonsnummer), $2))
       FROM public.search_employers(p_kommune_query => $3::text, p_limit => 100)'
      INTO v_n, v_h USING ',', '', cases[i][2];
    INSERT INTO _se_snap_after(label, n, h) VALUES (cases[i][1], v_n, v_h);
  END LOOP;

  FOR diff IN
    SELECT b.label, b.n AS n_before, a.n AS n_after, b.h AS h_before, a.h AS h_after
    FROM _se_snap_before b JOIN _se_snap_after a USING (label)
    WHERE b.n IS DISTINCT FROM a.n OR b.h IS DISTINCT FROM a.h
  LOOP
    RAISE EXCEPTION 'M5.4.1 treffsett-snapshot AVVIK for %: before(n=%, h=%) after(n=%, h=%)',
      diff.label, diff.n_before, diff.h_before, diff.n_after, diff.h_after;
  END LOOP;
END $do$;
