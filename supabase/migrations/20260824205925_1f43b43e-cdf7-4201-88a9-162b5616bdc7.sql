CREATE INDEX IF NOT EXISTS idx_source_postings_thread
  ON public.source_postings (identity_thread_id);

CREATE INDEX IF NOT EXISTS idx_source_postings_careerjet_published
  ON public.source_postings (published_at)
  WHERE source = 'careerjet';

CREATE INDEX IF NOT EXISTS idx_source_postings_superseded_by
  ON public.source_postings (identity_superseded_by_source_posting_id)
  WHERE identity_superseded_by_source_posting_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_careerjet_identity_audit_posting
  ON public.careerjet_identity_audit (source_posting_id);

CREATE INDEX IF NOT EXISTS idx_careerjet_observations_thread
  ON public.careerjet_source_observations (thread_id);
...
BEGIN
  ...
  -- Tråder der samtlige gjenværende annonser er med i denne porsjonen.
