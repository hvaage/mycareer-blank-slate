CREATE OR REPLACE FUNCTION public.search_employers(
  p_query text DEFAULT NULL::text, p_fylkesnummer text DEFAULT NULL::text,
  p_kommunenummer text DEFAULT NULL::text, p_naeringskode_prefix text DEFAULT NULL::text,
  p_min_ansatte integer DEFAULT NULL::integer, p_max_ansatte integer DEFAULT NULL::integer,
  p_min_omsetning numeric DEFAULT NULL::numeric, p_max_omsetning numeric DEFAULT NULL::numeric,
  p_arbeidsgiver_type text DEFAULT NULL::text, p_limit integer DEFAULT 25,
  p_offset integer DEFAULT 0, p_bransje_query text DEFAULT NULL::text,
...
BEGIN
  ...
  v_rank := CASE WHEN v_q IS NOT NULL
    THEN format('CASE WHEN e.organisasjonsnummer = %L THEN 2 ELSE similarity(e.navn, %L) END DESC,', v_qd, v_q)
    ELSE '' END;
  ...
  IF v_mirror THEN
    v_cand := format($c$
      ...
      ORDER BY
        CASE WHEN m.organisasjonsnummer = %L THEN 2 ELSE similarity(m.navn, %L) END DESC,
        m.antall_ansatte DESC NULLS LAST,
        m.navn ASC
      LIMIT %s
    $c$, v_qd, '%' || v_q || '%', v_qd, v_q, v_off + v_lim);
  ELSE
    v_cand := format($c$
...
CREATE OR REPLACE FUNCTION public.count_employers(
  p_query text DEFAULT NULL::text, p_fylkesnummer text DEFAULT NULL::text,
  p_kommunenummer text DEFAULT NULL::text, p_naeringskode_prefix text DEFAULT NULL::text,
  p_min_ansatte integer DEFAULT NULL::integer, p_max_ansatte integer DEFAULT NULL::integer,
  p_min_omsetning numeric DEFAULT NULL::numeric, p_max_omsetning numeric DEFAULT NULL::numeric,
  p_arbeidsgiver_type text DEFAULT NULL::text, p_bransje_query text DEFAULT NULL::text,
  p_kommune_query text DEFAULT NULL::text, p_cap integer DEFAULT 10000)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'reg'
...
DECLARE
  f jsonb;
  v_cap int := LEAST(GREATEST(COALESCE(p_cap, 10000), 100), 50000);
  v_n bigint;
  v_plan jsonb;
  v_mirror boolean;
BEGIN
  ...
  IF (f->>'empty')::boolean THEN
    RETURN jsonb_build_object('total_count', 0, 'is_estimate', false, 'cap', v_cap);
  END IF;
  ...
  IF v_n <= v_cap THEN
    RETURN jsonb_build_object('total_count', v_n, 'is_estimate', false, 'cap', v_cap);
  END IF;

  IF v_mirror THEN
    EXECUTE format(
      'EXPLAIN (FORMAT JSON) SELECT 1 FROM reg.enheter_sok m WHERE (m.organisasjonsnummer = %L OR m.navn ILIKE %L)',
      f->>'q_digits', '%' || (f->>'q') || '%'
    ) INTO v_plan;
  ELSE
    EXECUTE format(
      'EXPLAIN (FORMAT JSON) SELECT 1 FROM reg.enheter e %s %s %s',
      f->>'join', f->>'where', f->>'cand_add'
    ) INTO v_plan;
  END IF;

  RETURN jsonb_build_object(
    'total_count', GREATEST(COALESCE((v_plan->0->'Plan'->>'Plan Rows')::bigint, v_cap), v_cap),
    'is_estimate', true,
    'cap', v_cap
  );
END;
