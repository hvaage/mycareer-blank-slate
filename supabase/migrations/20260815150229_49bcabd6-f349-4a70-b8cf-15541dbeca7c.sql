ALTER TABLE reg.brreg_full_staging ADD COLUMN IF NOT EXISTS merged_at timestamptz;
CREATE INDEX IF NOT EXISTS brreg_full_staging_pending_idx
  ON reg.brreg_full_staging (run_id) WHERE merged_at IS NULL;

CREATE OR REPLACE FUNCTION public.brreg_full_merge(p_run_id bigint, p_batch integer DEFAULT 50000)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
AS $function$
DECLARE upserted bigint := 0; missing bigint := 0; remaining bigint := 0; done boolean := false;
BEGIN
  WITH batch AS (
  ...
  RETURN jsonb_build_object('upserted', upserted, 'remaining', remaining, 'done', done, 'missing', missing);
END;
$function$;
