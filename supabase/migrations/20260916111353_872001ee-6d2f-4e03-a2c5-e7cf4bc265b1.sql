...
BEGIN
  ...
  v_tier_e := CASE WHEN v_q IS NULL THEN NULL ELSE
    ...
    || ' ELSE 1 END' END;
  -- Speilet har navnet forhåndsnormalisert; bruk kolonnen i stedet for å
  -- kalle reg.sok_norm() per rad.
  v_tier_m := CASE WHEN v_q IS NULL THEN NULL ELSE
      'CASE WHEN e.organisasjonsnummer = ' || quote_literal(v_qd) || ' THEN 5'
    || ' WHEN lower(e.navn) = ' || quote_literal(v_ql) || ' THEN 4'
    || ' WHEN e.navn_norm = ' || quote_literal(reg.sok_norm(v_q)) || ' THEN 4'
    || ' WHEN lower(e.navn) LIKE ' || quote_literal(v_ql || '%') || ' THEN 3'
    || ' WHEN lower(e.navn) LIKE ' || quote_literal('% ' || v_ql || '%') || ' THEN 2'
    || ' ELSE 1 END' END;
  v_rank := CASE WHEN v_q IS NOT NULL
    ...
    ELSE '' END;
  v_rank_m := CASE WHEN v_q IS NOT NULL
    THEN v_tier_m || ' DESC, e.antall_ansatte DESC NULLS LAST, similarity(e.navn, ' || quote_literal(v_q) || ') DESC,'
    ELSE '' END;
  ...
  IF v_mirror THEN
    v_cand := format($c$
      ...
      ORDER BY
        ...
        e.navn ASC
      LIMIT %s
    $c$, f->>'where', v_rank_m, v_off + v_lim);
  ELSE
    v_cand := format($c$
      SELECT e.organisasjonsnummer
      FROM reg.enheter e
