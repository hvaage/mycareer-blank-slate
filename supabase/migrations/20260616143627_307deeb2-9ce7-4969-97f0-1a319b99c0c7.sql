-- M4.5 perf round: latest-regnskap MV + indexed kommune/bransje resolution

-- 1) Materialized view: latest regnskap per organisasjonsnummer
DROP MATERIALIZED VIEW IF EXISTS reg.regnskap_siste_per_org;
CREATE MATERIALIZED VIEW reg.regnskap_siste_per_org AS
SELECT DISTINCT ON (organisasjonsnummer) *
FROM reg.regnskap
ORDER BY
  organisasjonsnummer,
  regnskapsaar DESC NULLS LAST,
  CASE WHEN regnskapstype = 'SELSKAP' THEN 0 ELSE 1 END,
  hentet_tidspunkt DESC NULLS LAST;

CREATE UNIQUE INDEX regnskap_siste_per_org_pk
  ON reg.regnskap_siste_per_org (organisasjonsnummer);
CREATE INDEX regnskap_siste_per_org_driftsinntekter_idx
  ON reg.regnskap_siste_per_org (driftsinntekter);
CREATE INDEX regnskap_siste_per_org_regnskapsaar_idx
  ON reg.regnskap_siste_per_org (regnskapsaar);

GRANT SELECT ON reg.regnskap_siste_per_org TO anon, authenticated, service_role;

-- 2) Refresh function (concurrent so reads aren't blocked)
CREATE OR REPLACE FUNCTION reg.refresh_regnskap_siste_per_org()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
EXCEPTION WHEN OTHERS THEN
  -- Fallback (non-concurrent) hvis konkurranseregel ikke kan oppfylles
  REFRESH MATERIALIZED VIEW reg.regnskap_siste_per_org;
END;
$$;
REVOKE ALL ON FUNCTION reg.refresh_regnskap_siste_per_org() FROM public;
GRANT EXECUTE ON FUNCTION reg.refresh_regnskap_siste_per_org() TO service_role;

-- 3) Index for kommune-tekst → kommunenummer
CREATE INDEX IF NOT EXISTS idx_kommune_fylke_kommunenavn_lower
  ON reg.kommune_fylke (lower(kommunenavn) text_pattern_ops);

-- 4) Patched search_employers
...
DECLARE
  ...
  v_sql            text;
BEGIN
  -- Bransje-synonymer: map til NACE-prefiks + (lange) tekstord
  IF v_bq IS NOT NULL THEN
    IF v_bq IN ('it','ikt','tech','teknologi') THEN
      v_nace_prefixes := ARRAY['62','63'];
      v_text_terms    := ARRAY['informasjonsteknologi','programmering','programvare','databehandling','konsulent','kommunikasjonsteknologi'];
    ELSIF v_bq IN ('bygg','anlegg','entreprenor','entreprenør','bygg og anlegg') THEN
      v_nace_prefixes := ARRAY['41','42','43'];
      v_text_terms    := ARRAY['bygg','anlegg','entreprenør','oppføring'];
    ELSIF v_bq IN ('helse','omsorg','helse og omsorg') THEN
      v_nace_prefixes := ARRAY['86','87','88'];
      v_text_terms    := ARRAY['helse','omsorg','sykehus','pleie','sykehjem'];
    ELSIF v_bq IN ('regnskap','revisjon','økonomi','okonomi') THEN
      v_nace_prefixes := ARRAY['69.20','69.2'];
      v_text_terms    := ARRAY['regnskap','revisjon','økonomi','bokføring'];
    ELSIF v_bq IN ('transport','logistikk') THEN
      v_nace_prefixes := ARRAY['49','52','53'];
      v_text_terms    := ARRAY['transport','logistikk','spedisjon','lager'];
    ELSIF v_bq IN ('energi','kraft','strøm','strom') THEN
      v_nace_prefixes := ARRAY['35'];
      v_text_terms    := ARRAY['energi','kraft','elektrisitet'];
    ELSIF v_bq IN ('barnehage','skole','utdanning') THEN
      v_nace_prefixes := ARRAY['85'];
      v_text_terms    := ARRAY['barnehage','skole','utdanning','undervisning'];
    ELSIF length(v_bq) >= 3 THEN
      -- Ukjent kategori, men langt nok for trigram
      v_text_terms := ARRAY[v_bq];
    ELSE
      -- For kort til trigram og ingen synonym-treff → tomt resultat for bransje
      v_text_terms := ARRAY[]::text[];
    END IF;
  END IF;

  -- Kommune-tekst → kommunenummer-sett (btree-vennlig)
  IF v_kq IS NOT NULL THEN
    ...
    ELSE
      ...
      IF v_kommunenrs IS NULL OR array_length(v_kommunenrs,1) IS NULL THEN
        -- Ingen kommune matcher → resultatet skal være tomt
        RETURN;
      END IF;
    END IF;
  END IF;

  -- Bygg WHERE dynamisk så planneren ser ekte indekserbare predikater
  ...
  -- Bransje: kombiner NACE-prefiks-OR med tekst-OR over indekserte trigram-kolonner
  IF v_bq IS NOT NULL THEN
    DECLARE
    ...
    BEGIN
      ...
      IF array_length(v_parts, 1) IS NULL THEN
        RETURN; -- bransje-query gir tomt sett
      END IF;
      v_where := v_where || ' AND (' || array_to_string(v_parts, ' OR ') || ') ';
    END;
  END IF;

  -- Omsetning- og arbeidsgivertype-filtre presses ned i kandidat-CTE der mulig
  ...
  -- Fase 1: kandidat-orgnr (indeksvennlige predikater), Fase 2: ekspander med MV + companies
  v_sql := format($q$
    WITH cand AS MATERIALIZED (
      SELECT e.organisasjonsnummer
      FROM reg.enheter e
      LEFT JOIN reg.regnskap_siste_per_org lr0
        ON lr0.organisasjonsnummer = e.organisasjonsnummer
      %s
        AND (%L::numeric IS NULL OR lr0.driftsinntekter >= %L::numeric)
        AND (%L::numeric IS NULL OR lr0.driftsinntekter <= %L::numeric)
      ORDER BY
        %s
        e.antall_ansatte DESC NULLS LAST,
        lr0.driftsinntekter DESC NULLS LAST,
        e.navn ASC
      LIMIT 300
    )
    SELECT
  ...
  $q$,
    v_where,
    p_min_omsetning, p_min_omsetning,
    p_max_omsetning, p_max_omsetning,
...
-- 5) Initial refresh + analyze
REFRESH MATERIALIZED VIEW reg.regnskap_siste_per_org;
ANALYZE reg.regnskap_siste_per_org;
ANALYZE reg.enheter;
