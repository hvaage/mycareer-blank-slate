CREATE OR REPLACE FUNCTION public.linkedin_reconciliation_minimize_stale(
  p_user_id uuid,
...
BEGIN
  WITH affected AS (
    ...
    WHERE p.user_id = p_user_id
      ...
      AND (
        (p_proposal_ids IS NOT NULL AND p.id = ANY(p_proposal_ids))
        OR (
          EXISTS (SELECT 1 FROM public.linkedin_reconciliation_proposal_sources s WHERE s.proposal_id = p.id)
          AND NOT EXISTS (
            SELECT 1
...
REVOKE EXECUTE ON FUNCTION public.linkedin_reconciliation_minimize_stale(uuid, uuid[]) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.linkedin_reconciliation_minimize_stale(uuid, uuid[]) TO service_role;

DROP FUNCTION IF EXISTS public.linkedin_reconciliation_minimize_stale(uuid);

-- linkedin_import_delete: fang opp berørte forslag FØR staging slettes (kaskade fjerner koblingene)
CREATE OR REPLACE FUNCTION public.linkedin_import_delete(p_import_id uuid, p_reason text DEFAULT 'user_delete'::text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO ''
...
BEGIN
  ...
  -- forslag som har belegg i staging som er i ferd med å forsvinne
  SELECT COALESCE(array_agg(DISTINCT s.proposal_id), ARRAY[]::uuid[])
  INTO v_candidate_proposals
  FROM public.linkedin_reconciliation_proposal_sources s
  WHERE s.user_id = v_import.user_id;
  ...
  v_stale_proposals := public.linkedin_reconciliation_minimize_stale(v_import.user_id, v_candidate_proposals);
