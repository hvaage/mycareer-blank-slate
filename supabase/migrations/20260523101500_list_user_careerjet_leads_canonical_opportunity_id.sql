-- Expose canonical_opportunity_id on Careerjet leads RPC for target atoms + client use.
-- PG cannot change RETURNS TABLE via CREATE OR REPLACE; drop first.
...
BEGIN
  ...
  SELECT
    ...
    uo.canonical_opportunity_id AS canonical_opportunity_id,
    uo.status,
  ...
  SELECT
    ...
    uj.id,
    jl.id,
    NULL::uuid AS canonical_opportunity_id,
    uj.status,
...
COMMENT ON FUNCTION public.list_user_careerjet_leads(text) IS
  'Careerjet leads for auth.uid(): canonical + legacy. Adds canonical_opportunity_id for target atoms / UI.';
