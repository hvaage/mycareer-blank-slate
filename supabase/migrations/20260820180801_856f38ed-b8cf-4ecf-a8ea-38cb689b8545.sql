...
BEGIN
  BEGIN
    ...
    SELECT count(*) INTO n FROM public.linkedin_reconciliation_proposals;
    v := v || jsonb_build_object('n','RLS: bruker A ser kun egne forslag','p', n = 3, 'd', 'synlige forslag: '||n);

    SELECT count(*) INTO n FROM public.linkedin_reconciliation_runs;
    v := v || jsonb_build_object('n','RLS: bruker A ser kun egne kjøringer','p', n = 2, 'd', 'synlige kjøringer: '||n);

    BEGIN
      UPDATE public.linkedin_reconciliation_proposals SET status = 'approved_for_promotion' WHERE id = p_create;
      v := v || jsonb_build_object('n','RLS: klienten kan ikke skrive status direkte','p',false,'d','UPDATE gikk gjennom');
    EXCEPTION WHEN insufficient_privilege THEN
      v := v || jsonb_build_object('n','RLS: klienten kan ikke skrive status direkte','p',true,'d','insufficient_privilege');
    END;
    ...
    SELECT count(*) INTO n FROM public.linkedin_reconciliation_proposals;
    v := v || jsonb_build_object('n','RLS: bruker B ser ingen av A sine forslag','p', n = 0, 'd','synlige forslag: '||n);
    ...
    BEGIN
      SELECT count(*) INTO n FROM public.linkedin_reconciliation_proposals;
      v := v || jsonb_build_object('n','anon: ingen leseadgang til forslag','p',false,'d','SELECT gikk gjennom, rader: '||n);
    EXCEPTION WHEN insufficient_privilege THEN
      v := v || jsonb_build_object('n','anon: ingen leseadgang til forslag','p',true,'d','insufficient_privilege');
    END;
    EXECUTE 'RESET ROLE';
    PERFORM set_config('request.jwt.claims', NULL, true);
