...
BEGIN
  BEGIN
    ...
    IF coalesce((r->>'ok')::boolean,false) AND v_a_rows = 1 AND v_a_status = 'target' AND v_a_prio = 'high' THEN
      t1 := 'PASS';
    END IF;
    t1d := format('rpc_ok=%s rows_A=%s status=%s priority=%s', r->>'ok', v_a_rows, v_a_status, v_a_prio);
    ...
    IF coalesce((r->>'ok')::boolean,true) = false AND (r->>'error_code') = 'batch_not_found'
      ...
      t3 := 'PASS';
    END IF;
    t3d := format('error_code=%s contacts=%s items_pending=%s', r->>'error_code', v_c, v_pending);
    ...
    IF v_err LIKE '%SYNTHETIC_FAILURE_MIDBATCH%' AND v_c = 0 AND v_i = 0 AND v_rel = 0 AND v_pending = 3 THEN
      t5 := 'PASS';
    END IF;
    t5d := format('feil=%s contacts=%s identities=%s relations=%s items_pending=%s',
