...
BEGIN
  ...
  IF before_metadata->'owner' IS DISTINCT FROM after_metadata->'owner'
     OR before_metadata->'security_definer' IS DISTINCT FROM after_metadata->'security_definer'
     OR before_metadata->'volatility' IS DISTINCT FROM after_metadata->'volatility'
     OR before_metadata->'parallel' IS DISTINCT FROM after_metadata->'parallel'
     OR before_metadata->'config' IS DISTINCT FROM after_metadata->'config'
     OR before_metadata->'acl' IS DISTINCT FROM after_metadata->'acl'
     OR before_metadata->'arguments' IS DISTINCT FROM after_metadata->'arguments'
     OR before_metadata->'result' IS DISTINCT FROM after_metadata->'result' THEN
    RAISE EXCEPTION 'list_user_job_opportunities(text,text) metadata changed: before=% after=%',
