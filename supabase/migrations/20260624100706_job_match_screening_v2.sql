...
BEGIN
  ...
  INSERT INTO public.job_match_evaluations (
    ...
    v_status, coalesce(p_result->'screening_reasons', '[]'::jsonb),
    coalesce(p_result->'requirement_summary', '{}'::jsonb), v_score,
    nullif(p_result->>'reasoning', ''), nullif(p_result->>'match_highlights', ''),
    nullif(p_result->>'concerns', ''), coalesce(v_previous, '{}'::jsonb),
    p_profile_input_hash, p_job_input_hash
  );
...
REVOKE ALL ON FUNCTION public.record_job_match_evaluation(
  uuid, text, uuid, jsonb, text, text, text, text
) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.record_job_match_evaluation(
  uuid, text, uuid, jsonb, text, text, text, text
) TO service_role;

COMMENT ON FUNCTION public.record_job_match_evaluation(
  uuid, text, uuid, jsonb, text, text, text, text
) IS 'Atomically replaces one user match result and appends its previous value to history. Service role only.';

COMMENT ON COLUMN public.user_opportunities.screening_status IS
  'V2 hard-screen result. Default UI should show only eligible rows.';

COMMENT ON COLUMN public.user_opportunities.screening_reasons IS
  'Structured exclusion or review reasons; never a replacement for the score.';

COMMENT ON COLUMN public.user_job_listing_status.screening_status IS
  'V2 hard-screen result for legacy listings during canonical transition.';

COMMENT ON COLUMN public.opportunity_requirement_atoms.requirement_level IS
  'Whether the requirement is mandatory, preferred, or contextual.';
