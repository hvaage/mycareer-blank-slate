-- Speilet dekker nå også sted-, bransjekode- og ansattefiltre. Kriteriet for å
-- bruke det flyttes derfor fra "ingen filtre" til "ingen filtre som krever
-- kolonner speilet ikke har" (fritekst-bransje, omsetning, arbeidsgivertype).
...
BEGIN
  ...
  -- Speilet har navn, orgnr, ansatte, kommune, fylke og næringskode1.
  -- Fritekst-bransje, omsetning og arbeidsgivertype krever hele reg.enheter.
  v_mirror := v_q IS NOT NULL
    AND p_min_omsetning IS NULL AND p_max_omsetning IS NULL
    AND p_arbeidsgiver_type IS NULL AND p_bransje_query IS NULL;

  IF v_mirror THEN
    v_cand := format($c$
      SELECT e.organisasjonsnummer
      FROM reg.enheter_sok e
      %s
      ORDER BY
        %s
        e.navn ASC
      LIMIT %s
    $c$, f->>'where', v_rank, v_off + v_lim);
  ELSE
    v_cand := format($c$
      SELECT e.organisasjonsnummer
      FROM reg.enheter e
...
BEGIN
  ...
  IF v_mirror THEN
    EXECUTE format(
      'SELECT count(*) FROM (SELECT 1 FROM reg.enheter_sok e %s LIMIT %s) t',
      f->>'where', v_cap + 1
    ) INTO v_n;
  ELSE
    EXECUTE format(
...
CREATE OR REPLACE FUNCTION public.employer_ansatte_distribution(p_query text DEFAULT NULL::text, p_fylkesnummer text DEFAULT NULL::text, p_kommunenummer text DEFAULT NULL::text, p_naeringskode_prefix text DEFAULT NULL::text, p_min_omsetning numeric DEFAULT NULL::numeric, p_max_omsetning numeric DEFAULT NULL::numeric, p_arbeidsgiver_type text DEFAULT NULL::text, p_bransje_query text DEFAULT NULL::text, p_kommune_query text DEFAULT NULL::text, p_cap integer DEFAULT 50000)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'reg'
...
BEGIN
  ...
  IF v_mirror THEN
    v_limit := v_cap + 1;
    v_src := format(
      'SELECT e.antall_ansatte, e.har_registrert_antall_ansatte FROM reg.enheter_sok e %s',
      f->>'where');
  ELSE
    v_limit := v_filter_cap;
    v_src := format(
      'SELECT e.antall_ansatte, e.har_registrert_antall_ansatte FROM reg.enheter e %s %s %s',
