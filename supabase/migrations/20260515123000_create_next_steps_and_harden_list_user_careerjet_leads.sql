-- 1) Create public.next_steps (referenced by app; prior migrations only indexed it if it existed).
-- 2) Harden list_user_careerjet_leads: normalize p_status, avoid ambiguous filters when p_status is null/blank.
-- 3) Replace list_user_careerjet_leads body (RETURNS TABLE includes ai_scored_at).
-- 4) Ask PostgREST to reload schema so /rpc/list_user_careerjet_leads matches the new return type.
