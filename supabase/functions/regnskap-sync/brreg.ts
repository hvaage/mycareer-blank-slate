...
const MAX_REGNSKAP_ATTEMPTS = 2;
const MAX_PDF_YEAR_ATTEMPTS = 2;
...
export function shouldRetryImmediately(status: number): boolean {
...
export async function fetchRegnskap(
  orgnr: string,
  limiter: RateLimiter,
): Promise<BrregFetchResult> {
  let attempts = 0, http429 = 0, http503 = 0, totalMs = 0;
  let lastErr = "", lastStatus: number | null = null;
  for (let i = 0; i < MAX_REGNSKAP_ATTEMPTS; i++) {
    attempts++;
    await limiter.take();
    const { res, ms, err } = await timedFetch(
      `${BASE}/${encodeURIComponent(orgnr)}`,
    );
    totalMs += ms;
    if (!res) {
      lastErr = err ?? "network";
      lastStatus = null;
      await sleepBeforeRetry(i, MAX_REGNSKAP_ATTEMPTS);
      continue;
    }
    ...
    lastErr = `http ${res.status}`;
    if (!shouldRetryImmediately(res.status)) break;
    await sleepBeforeRetry(i, MAX_REGNSKAP_ATTEMPTS);
  }
  return {
    kind: "retry_exhausted",
...
export async function fetchPdfYears(
  orgnr: string,
  limiter: RateLimiter,
): Promise<PdfYearsResult> {
  let attempts = 0, totalMs = 0;
  let lastStatus: number | null = null, lastErr = "";
  for (let i = 0; i < MAX_PDF_YEAR_ATTEMPTS; i++) {
    attempts++;
    await limiter.take();
    const { res, ms, err } = await timedFetch(
      `${BASE}/aarsregnskap/kopi/${encodeURIComponent(orgnr)}/aar`,
    );
    totalMs += ms;
    if (!res) {
      lastErr = err ?? "network";
      await sleepBeforeRetry(i, MAX_PDF_YEAR_ATTEMPTS);
      continue;
    }
    lastStatus = res.status;
    if (res.status === 200) {
      try {
        ...
        const years = Array.isArray(body)
          ? (body as unknown[]).map((v) => Number(v)).filter((v) =>
            Number.isInteger(v)
          )
          : [];
    ...
    if (res.status === 404) {
      return { kind: "none", years: [], latencyMs: totalMs, attempts };
    }
  ...
  return {
    ...
    attempts,
    status: lastStatus,
    error: lastErr || "pdf-years failed",
  };
}
