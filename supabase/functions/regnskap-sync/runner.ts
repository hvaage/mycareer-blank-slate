...
import { regnskapRad, type RegnskapRow, sampleOk } from "./normalize.ts";
import { fetchPdfYears, fetchRegnskap, RateLimiter } from "./brreg.ts";
import {
  analyzeRegnskapTables,
  type ClaimedOrg,
  claimOrgs,
  ensureStatusRows,
  type FinalStatus,
  ...
  insertRunItems,
  patchRunMeta,
  refreshLatestRegnskapMV,
  releaseClaim,
  releaseRegnskapSyncLock,
  type RunItem,
  selectCandidates,
  startRun,
  type SyncMode,
...
import { OBSERVE_VARIANTS, warmupSearch } from "./warmup.ts";
import { StageError, tagStage } from "./_stage.ts";
...
export async function runSync(input: RunSyncInput): Promise<RunSyncResult> {
  ...
  try {
    return await tagStage("db_connect", () =>
      withClient(async (c) => {
        let lockAcquired = false;
        if (!dryRun) {
          lockAcquired = await tagStage("acquire_lock", () =>
            tryAcquireRegnskapSyncLock(c));
          if (!lockAcquired) {
            result.stoppedReason = "already_running";
            result.durationMs = Date.now() - t0;
            const skippedRunId = await tagStage("start_run", () =>
              startRun(c, {
                mode,
                dryRun,
                limit,
        ...
        try {
          const candidates = await tagStage("select_candidates", () =>
            selectCandidates(c, mode, limit, staleDays, input.orgnrs));
          if (candidates.length === 0) {
            result.durationMs = Date.now() - t0;
            if (!dryRun) {
              runId = await tagStage("start_run", () =>
                startRun(c, {
                  mode,
                  dryRun,
                  limit,
          ...
          if (!dryRun) {
            await tagStage("ensure_status", () =>
              ensureStatusRows(c, candidates));
          }

          let claimed: ClaimedOrg[];
          if (dryRun) {
            claimed = candidates.slice(0, limit).map((o) => ({
              organisasjonsnummer: o,
              prevStatus: null,
            }));
          } else {
            claimed = await tagStage("claim", () =>
              claimOrgs(c, candidates, limit, mode));
          }
          ...
          for (let i = 0; i < claimed.length; i++) {
            ...
            if (Date.now() - t0 > stopThresholdMs) {
              ...
              if (!dryRun) {
                for (let j = i; j < claimed.length; j++) {
                  await releaseClaim(
                    c,
                    claimed[j].organisasjonsnummer,
                    claimed[j].prevStatus,
                  );
                  result.skipped++;
                }
              }
            ...
            const fetchRes = await tagStage("brreg_fetch", () =>
              fetchRegnskap(orgnr, limiter));
            ...
            if (fetchRes.kind === "ok") {
              normalized = fetchRes.data.map((rr) =>
                regnskapRad(orgnr, rr)
              );
              const yrs = normalized.map((r) =>
                r.regnskapsaar
            ...
            if (
              includePdfYears &&
              (finalStatus === "ok" || finalStatus === "no_regnskap")
            ) {
              const pdf = await tagStage("brreg_fetch", () =>
                fetchPdfYears(orgnr, limiter));
              if (pdf.kind === "ok") {
                pdfYears = pdf.years;
              } else if (pdf.kind === "none") {
                pdfYears = [];
              }
            }
          ...
          if (!dryRun && runId !== null) {
            try {
              await tagStage("insert_run_items", () =>
                insertRunItems(c, runItems));
            } catch (e) {
              lastErr = e instanceof Error ? e.message : String(e);
            }
            result.durationMs = Date.now() - t0;
            const runStatus =
              (result.stoppedReason !== "done" || result.failed > 0)
                ? "partial"
                : "ok";
            await tagStage("finish_run", () =>
              finishRun(c, {
            ...
            if (result.checked > 0) {
              ...
              try {
                ...
                const analyze = await analyzeRegnskapTables(c, {
                  includeEnheter,
                });
              ...
              try {
                const warmup = await warmupSearch(c, {
          ...
          if (lockAcquired) {
            try {
              await tagStage("release_lock", () =>
                releaseRegnskapSyncLock(c));
            } catch (e) {
              console.error(
                "[runner] releaseRegnskapSyncLock failed:",
                e instanceof Error ? e.message : String(e),
              );
            }
          }
    ...
    if (e instanceof StageError) (e as any).runId = runId;
      ...
      const se = new StageError("unknown", e);
      (se as any).runId = runId;
      throw se;
    }
    throw e;
  }
