...
export function findDuplicates(
  ...
  for (const inc of incoming) {
    const candidates: DuplicatePair<AtomInsert, CvAtom>[] = [];
    for (const ex of existing) {
      if (inc.atom_type !== ex.atom_type) continue;
      const match = compareAtoms(inc, ex);
      if (match) {
        candidates.push({ incoming: inc, existing: ex, ...match });
      }
    }
    candidates.sort((a, b) => b.confidence - a.confidence);
    if (candidates[0]) pairs.push(candidates[0]);
  }
...
export function mergeAtoms(
  ...
  const proposal = proposeAtomMerge(existing, incoming);
  return proposal.safety === "safe_fill_missing" ? proposal.patch : {};
}

export function proposeAtomMerge(
  existing: CvAtom,
  incoming: AtomInsert,
): MergeProposal {
  if (existing.user_locked) {
    return {
      safety: "blocked_user_locked",
      patch: {},
      reason: "Eksisterende atom er bruker-låst og kan ikke endres av import eller AI.",
    };
  }

  const preserveExisting = existing.user_confirmed || existing.confidence === "verified";
  const merged = {
    content_no: preserveExisting
      ? existing.content_no ?? incoming.content_no
      : pickLonger(existing.content_no, incoming.content_no),
    content_en: preserveExisting
      ? existing.content_en ?? incoming.content_en
      : pickLonger(existing.content_en, incoming.content_en),
    ...
    confidence: preserveExisting ? existing.confidence : pickHigherConfidence(
      existing.confidence,
  ...
  return {
    safety: preserveExisting ? "review_required" : "safe_fill_missing",
    patch: merged as unknown as Partial<CvAtom>,
    reason: preserveExisting
      ? "Brukerbekreftet fakta bevares; foreslåtte tillegg må godkjennes."
      : "Importert atom kan berikes med manglende kildeverdier.",
  };
}
...
function compareAchievements(
  ...
  const aSemanticKey = (inc.structured_data as Partial<AchievementStructuredData>)?.semantic_key;
  const bSemanticKey = (ex.structured_data as AchievementStructuredData)?.semantic_key;
  if (aSemanticKey && bSemanticKey && aSemanticKey === bSemanticKey) {
    return {
      reason: "Samme semantiske nøkkel under samme rolle",
      confidence: 0.96,
    };
  }
