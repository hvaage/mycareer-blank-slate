...
Deno.test("kildehashen er deterministisk og endres med innhold", async () => {
  const importId = "33333333-3333-4333-8333-333333333333";
  const h1 = await computeSourceHash(importId, buildSegments([a, b]), "1.0.0");
  const h2 = await computeSourceHash(importId, buildSegments([b, a]), "1.0.0");
  assertEquals(h1, h2);

  const changed = { ...b, content_no: "Kuttet kostnader med 15 prosent" };
  const h3 = await computeSourceHash(importId, buildSegments([a, changed]), "1.0.0");
