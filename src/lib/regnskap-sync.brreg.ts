/**
 * Brreg-fetch med retry/backoff og lokal rate-limit (token-bucket).
 * Server-only — bruker fetch. Ingen client-import.
 */
...
export type PdfYearsResult =
  ...
  | { kind: "none"; years: []; latencyMs: number; attempts: number }
...
export async function fetchRegnskap(orgnr: string, limiter: RateLimiter): Promise<BrregFetchResult> {
  ...
  for (let i = 0; i < MAX_ATTEMPTS; i++) {
    ...
    if (res.status === 200) {
      ...
      if (arr.length === 0) {
        return { kind: "no_regnskap", status: 200, latencyMs: totalMs, attempts, http429, http503 };
      }
      return { kind: "ok", status: 200, data: arr, latencyMs: totalMs, attempts, http429, http503 };
    }
    if (res.status === 404) {
      return { kind: "not_found", status: 404, latencyMs: totalMs, attempts, http429, http503 };
    }
    if (res.status === 403) {
      return { kind: "forbidden", status: 403, latencyMs: totalMs, attempts, http429, http503 };
    }
    if (res.status === 429) {
      http429++;
      lastErr = `http 429`;
    ...
    if (res.status === 200) {
      try {
        ...
        const years = Array.isArray(body)
          ? (body as unknown[]).map((v) => Number(v)).filter((v) => Number.isInteger(v))
          : [];
