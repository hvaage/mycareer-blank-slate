-- Fase 3 — LinkedIn-avstemming: syntetisk canary.
--
-- Kjøres med:  SELECT * FROM public.linkedin_reconciliation_phase3_canary();
--
-- Hele testen kjører i en indre subtransaksjon som ALLTID rulles tilbake.
-- Ingen syntetiske rader blir liggende igjen, og ingen produktdata røres.
-- Funksjonen bruker kun syntetiske bruker-UUID-er (ikke reelle LinkedIn-data).
...
BEGIN
  BEGIN
    ...
    -- ---------- 1. idempotens: identisk inputsignatur ----------
    BEGIN
      INSERT INTO public.linkedin_reconciliation_runs
    ...
    -- ny regelversjon => ny signatur => ny kjøring tillatt
    INSERT INTO public.linkedin_reconciliation_runs
    ...
    -- ---------- 3. kryssbruker-isolasjon i datamodellen ----------
    BEGIN
      INSERT INTO public.linkedin_reconciliation_proposal_sources
    ...
    -- ---------- 4. tilgangskontroll (grants + policyer) ----------
    ...
    -- ---------- 5. beslutningskontrakt (som bruker A) ----------
    ...
    -- klienten kan ikke endre eller slette historikk
    BEGIN
    ...
    -- ---------- 6. kryssbruker via RPC (som bruker B) ----------
    ...
    -- ---------- 7. retention: sletting minimerer forslaget ----------
    ...
    -- stale_source er ikke handlingsbart
    PERFORM set_config('request.jwt.claims', json_build_object('sub', u_a, 'role', 'authenticated')::text, true);
