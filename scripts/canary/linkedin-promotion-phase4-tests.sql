...
BEGIN
  BEGIN
    ...
    SELECT status INTO res FROM (SELECT to_jsonb(status) AS status FROM public.linkedin_reconciliation_proposals WHERE id = p_rec) s;
    ...
    -- feilhendelsen skrives separat, som revisjon
    ...
    -- ikke-retrybar feil låser forslaget og kan gjenåpnes
